...
async function cfFetch(path: string, opts: RequestInit = {}): Promise<unknown> {
  ...
  if (!token && !(email && key)) throw new Error('CLOUDFLARE_API_TOKEN or CLOUDFLARE_EMAIL+CLOUDFLARE_GLOBAL_API_KEY not set');

  const headers: Record<string, string> = { 'Content-Type': 'application/json' };
  if (token) headers['Authorization'] = `Bearer ${token}`;
  else { headers['X-Auth-Email'] = email!; headers['X-Auth-Key'] = key!; }
...
export async function listZones(): Promise<Array<{ id: string; name: string; status: string }>> {
  const res = await cfFetch('/zones?per_page=50') as Array<{ id: string; name: string; status: string }>;
  return res;
}
...
export async function listDnsRecords(
  ...
  return cfFetch(`/zones/${zoneId}/dns_records?per_page=100`) as Promise<Array<{ id: string; type: string; name: string; content: string }>>;
}

export async function createDnsRecord(
  zoneId: string,
  type: string, name: string, content: string, ttl = 1,
): Promise<{ id: string }> {
...
export async function listWorkers(): Promise<Array<{ id: string; etag: string }>> {
  return cfFetch(`/accounts/${ACCOUNT_ID()}/workers/scripts`) as Promise<Array<{ id: string; etag: string }>>;
}
