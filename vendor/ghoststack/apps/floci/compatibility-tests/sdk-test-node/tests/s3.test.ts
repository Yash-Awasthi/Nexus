...
describe("S3", () => {
  ...
  it("should put object", async () => {
    await s3.send(new PutObjectCommand({ Bucket: bucketName, Key: "test.txt", Body: "hello from test" }));
  });

  it("should head object", async () => {
    const response = await s3.send(new HeadObjectCommand({ Bucket: bucketName, Key: "test.txt" }));
    expect(response.ContentLength).toBeGreaterThan(0);
    expect(response.LastModified).toBeInstanceOf(Date);
  });

  it("should get object", async () => {
    const response = await s3.send(new GetObjectCommand({ Bucket: bucketName, Key: "test.txt" }));
  ...
  it("should copy object with non-ASCII key", async () => {
    ...
    const response = await s3.send(new GetObjectCommand({ Bucket: bucketName, Key: nonAsciiDst }));
    const body = await response.Body?.transformToString();
  ...
  it("should multipart copy object with non-ASCII key", async () => {
    ...
    try {
      ...
      const response = await s3.send(new GetObjectCommand({ Bucket: bucketName, Key: dstKey }));
  ...
  it("should upload large object (25 MB)", async () => {
    ...
    const response = await s3.send(new HeadObjectCommand({ Bucket: bucketName, Key: "large-object-25mb.bin" }));
    expect(response.ContentLength).toBe(25 * 1024 * 1024);

    await s3.send(new DeleteObjectCommand({ Bucket: bucketName, Key: "large-object-25mb.bin" }));
  });
  ...
  it("should fail for missing object", async () => {
    await expect(s3.send(new GetObjectCommand({ Bucket: bucketName, Key: "missing.txt" }))).rejects.toThrow();
  });
