...
describe("Cognito", () => {
  ...
  it("should create user pool", async () => {
    const response = await cognito.send(new CreateUserPoolCommand({ PoolName: `test-pool-${uniqueName()}` }));
  ...
  it("should admin get user", async () => {
    const response = await cognito.send(new AdminGetUserCommand({ UserPoolId: poolId, Username: "testuser" }));
  ...
  it("should get group", async () => {
    const response = await cognito.send(new GetGroupCommand({ UserPoolId: poolId, GroupName: "test-group" }));
    expect(response.Group?.GroupName).toBe("test-group");
  ...
  it("should fail to create duplicate group", async () => {
    await expect(
      cognito.send(new CreateGroupCommand({ UserPoolId: poolId, GroupName: "test-group" }))
    ).rejects.toThrow();
  });
  ...
  it("should delete group", async () => {
    await cognito.send(new DeleteGroupCommand({ UserPoolId: poolId, GroupName: "test-group" }));
  });

  it("should fail to get deleted group", async () => {
    await expect(cognito.send(new GetGroupCommand({ UserPoolId: poolId, GroupName: "test-group" }))).rejects.toThrow();
  });
