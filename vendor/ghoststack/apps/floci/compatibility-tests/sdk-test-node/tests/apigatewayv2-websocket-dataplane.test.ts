...
describe("WebSocket Data-Plane", () => {
  ...
  describe("Chat-style broadcast", () => {
    ...
    beforeAll(async () => {
      const broadcastFn = await createLambda(
        "broadcast",
        BROADCAST_HANDLER,
        {
  ...
  describe("Mock integration", () => {
    ...
    beforeAll(async () => {
      ...
      const mockIntegRes = await gw.send(
        new CreateIntegrationCommand({
          ApiId: apiId,
...
`;
    ...
    beforeAll(async () => {
      disconnectFnName = await createLambda(
        "disconnect-fn",
        DISCONNECT_HANDLER,
        {
          FLOCI_ENDPOINT: "http://host.docker.internal:4566"
