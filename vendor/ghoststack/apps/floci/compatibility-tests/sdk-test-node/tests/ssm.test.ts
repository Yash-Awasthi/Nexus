...
describe("SSM", () => {
  ...
  it("should put and get a String parameter", async () => {
    ...
    await ssm.send(new PutParameterCommand({ Name: name, Value: "hello", Type: "String" }));
  ...
  it("should put and get a SecureString parameter", async () => {
    ...
    await ssm.send(new PutParameterCommand({ Name: name, Value: "s3cr3t", Type: "SecureString" }));
  ...
  it("should get parameters by path", async () => {
    const response = await ssm.send(new GetParametersByPathCommand({ Path: paramPath, Recursive: true }));
  ...
  it("should fail for missing parameter", async () => {
    await expect(ssm.send(new GetParameterCommand({ Name: `${paramPath}/missing` }))).rejects.toThrow();
  });
