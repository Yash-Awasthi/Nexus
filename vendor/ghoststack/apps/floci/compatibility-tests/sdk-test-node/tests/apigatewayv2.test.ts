...
describe("API Gateway v2", () => {
  ...
  describe("HTTP API lifecycle", () => {
    ...
    afterAll(async () => {
      try {
        if (apiId) await gw.send(new DeleteApiCommand({ ApiId: apiId }));
      } catch {
        /* ignore */
      }
    });
  ...
  describe("WebSocket API lifecycle", () => {
    ...
    afterAll(async () => {
      try {
        if (wsApiId) await gw.send(new DeleteApiCommand({ ApiId: wsApiId }));
      } catch {
        /* ignore */
      }
    });
  ...
  describe("Integrations", () => {
    ...
    it("should delete the integration", async () => {
      ...
      await expect(
        gw.send(new GetIntegrationCommand({ ApiId: apiId, IntegrationId: integrationId }))
      ).rejects.toThrow();
    });
  });
  ...
  describe("Stages & Deployments", () => {
    ...
    afterAll(async () => {
      try {
        await gw.send(new DeleteStageCommand({ ApiId: apiId, StageName: "dev" }));
      } catch {
        /* ignore */
      }
      try {
