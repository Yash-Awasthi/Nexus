...
describe("Secrets Manager", () => {
  ...
  it("should create secret", async () => {
    const response = await sm.send(new CreateSecretCommand({ Name: secretName, SecretString: '{"key":"value"}' }));
  ...
  it("should update secret", async () => {
    await sm.send(new UpdateSecretCommand({ SecretId: secretName, SecretString: '{"key":"updated"}' }));
  ...
  it("should fail to get deleted secret", async () => {
    await expect(sm.send(new GetSecretValueCommand({ SecretId: `test/secret/${uniqueName()}` }))).rejects.toThrow();
  });
});
