...
describe("S3 CORS Enforcement", () => {
  ...
  async function rawRequest(method: string, path: string, extraHeaders: Record<string, string> = {}) {
  ...
  beforeAll(async () => {
    ...
    await s3.send(new PutObjectCommand({ Bucket: bucketName, Key: "cors-test.txt", Body: "hello cors" }));
  });
