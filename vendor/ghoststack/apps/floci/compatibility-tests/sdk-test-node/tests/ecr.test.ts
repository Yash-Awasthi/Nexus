...
describe("ECR Repository Lifecycle", () => {
  ...
  it("CreateRepository on duplicate name throws RepositoryAlreadyExistsException", async () => {
    await expect(ecr.send(new CreateRepositoryCommand({ repositoryName: REPO_NAME }))).rejects.toBeInstanceOf(
      RepositoryAlreadyExistsException
    );
  });

  it("DescribeRepositories returns the created repository", async () => {
    const resp = await ecr.send(new DescribeRepositoriesCommand({ repositoryNames: [REPO_NAME] }));
  ...
  it("PutLifecyclePolicy round-trips the policy text", async () => {
    ...
    await ecr.send(new PutLifecyclePolicyCommand({ repositoryName: REPO_NAME, lifecyclePolicyText: policy }));
  ...
  it("SetRepositoryPolicy round-trips the policy text", async () => {
    ...
    await ecr.send(new SetRepositoryPolicyCommand({ repositoryName: REPO_NAME, policyText: policy }));
  ...
  it("DeleteRepository force=true removes the repository", async () => {
    await ecr.send(new DeleteRepositoryCommand({ repositoryName: REPO_NAME, force: true }));
    await expect(ecr.send(new DescribeRepositoriesCommand({ repositoryNames: [REPO_NAME] }))).rejects.toBeInstanceOf(
      RepositoryNotFoundException
    );
  });
  ...
  it("DescribeRepositories on missing name throws RepositoryNotFoundException", async () => {
    await expect(
      ecr.send(new DescribeRepositoriesCommand({ repositoryNames: ["does-not-exist-node"] }))
