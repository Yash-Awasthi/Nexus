...
describe("DynamoDB — Phase 1: ValidationException rename", () => {
  ...
  it("rejects a 2-char table name with ValidationException", async () => {
    await expectError(ddb.send(new PutItemCommand({ TableName: "ab", Item: { pk: s("x") } })), "ValidationException");
  });

  it("rejects empty table name with ValidationException", async () => {
    await expectError(ddb.send(new GetItemCommand({ TableName: "", Key: { pk: s("x") } })), "ValidationException");
  });
});
...
describe("DynamoDB — Phase 3: Batch limits & key validation", () => {
  ...
  afterAll(async () => {
    ...
    try {
      await ddb.send(new DeleteTableCommand({ TableName: lsiTable }));
    } catch {
      /* ignore */
    }
  });
...
describe("DynamoDB — Phase 4: Query/Scan behaviour", () => {
  ...
  it("parallel scan requires both Segment and TotalSegments", async () => {
    await expectError(ddb.send(new ScanCommand({ TableName: table, Segment: 0 })), "ValidationException");
    await expectError(ddb.send(new ScanCommand({ TableName: table, TotalSegments: 2 })), "ValidationException");
  });
  ...
  it("Query without KeyConditionExpression throws ValidationException", async () => {
    await expectError(ddb.send(new QueryCommand({ TableName: table })), "ValidationException");
  });
});
