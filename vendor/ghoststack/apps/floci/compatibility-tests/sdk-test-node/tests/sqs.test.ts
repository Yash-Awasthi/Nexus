...
describe("SQS", () => {
  ...
  afterAll(async () => {
    ...
    try {
      await sqs.send(new DeleteQueueCommand({ QueueUrl: `${ENDPOINT}/${ACCOUNT}/${fifoQueueName}` }));
    } catch {
      // ignore
    }
  });
  ...
  it("should get queue attributes", async () => {
    const response = await sqs.send(new GetQueueAttributesCommand({ QueueUrl: queueUrl, AttributeNames: ["All"] }));
    expect(response.Attributes?.QueueArn).toBeTruthy();
  });

  it("should send and receive message", async () => {
    await sqs.send(new SendMessageCommand({ QueueUrl: queueUrl, MessageBody: "test-message" }));
