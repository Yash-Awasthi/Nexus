...
describe("Lambda", () => {
  ...
  it("should publish version", async () => {
    const response = await lambda.send(new PublishVersionCommand({ FunctionName: fnName, Description: "v1" }));
  ...
  it("should get alias", async () => {
    const response = await lambda.send(new GetAliasCommand({ FunctionName: fnName, Name: "live" }));
  ...
  it("should fail to get deleted alias", async () => {
    await expect(lambda.send(new GetAliasCommand({ FunctionName: fnName, Name: "live" }))).rejects.toThrow();
  });
  ...
  it("should fail to get deleted function", async () => {
    await expect(lambda.send(new GetFunctionCommand({ FunctionName: `test-fn-${uniqueName()}` }))).rejects.toThrow();
  });
});
...
describe("Lambda ImageConfig.WorkingDirectory", () => {
  ...
  it("round-trips WorkingDirectory through create and get", async () => {
    ...
    try {
      const createResp = await lambda.send(
        new CreateFunctionCommand({
          FunctionName: fnName,
          PackageType: "Image",
          Role: ROLE,
  ...
  it("updates WorkingDirectory via updateFunctionConfiguration", async () => {
    ...
    try {
      ...
      const updateResp = await lambda.send(
