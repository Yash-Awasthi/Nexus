...
describe("KMS", () => {
  ...
  it("should create key", async () => {
    const response = await kms.send(new CreateKeyCommand({ Description: `test-key-${uniqueName()}` }));
  ...
  it("should generate data key", async () => {
    const response = await kms.send(new GenerateDataKeyCommand({ KeyId: keyId, KeySpec: "AES_256" }));
