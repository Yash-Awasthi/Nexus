...
describe("SNS", () => {
  ...
  beforeAll(async () => {
    ...
    const queueResult = await sqs.send(new CreateQueueCommand({ QueueName: `sns-target-${uniqueName()}` }));
  ...
  it("should create a topic", async () => {
    const response = await sns.send(new CreateTopicCommand({ Name: `test-topic-${uniqueName()}` }));
  ...
  it("should list subscriptions by topic", async () => {
    const response = await sns.send(new ListSubscriptionsByTopicCommand({ TopicArn: topicArn }));
    expect(response.Subscriptions?.length).toBeGreaterThan(0);
  });

  it("should get subscription attributes", async () => {
    const response = await sns.send(new GetSubscriptionAttributesCommand({ SubscriptionArn: subscriptionArn }));
  ...
  it("should publish message", async () => {
    const response = await sns.send(new PublishCommand({ TopicArn: topicArn, Message: "test-sns-message" }));
