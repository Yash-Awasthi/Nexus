...
export async function briefRoutes(app: FastifyInstance): Promise<void> {
  ...
  }>("/brief/:domain", { preHandler: requireAuth }, async (request, reply) => {
  ...
  }>("/brief/:domain/events", { preHandler: requireAuth }, async (request, reply) => {
    const { domain } = request.params;
    const {
      events,
      userId = "default",
  ...
  app.get<{
    ...
    const fullUrl = url ?? (request.hostname
