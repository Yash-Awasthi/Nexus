...
export async function gatewayRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{
    ...
    const totalUsd = allRuns.reduce(
      (s, r) => s + r.steps.reduce((a, st) => a + (st.costUsd ?? 0), 0),
      0,
    );
    const totalTok = allRuns.reduce(
      (s, r) => s + r.steps.reduce((a, st) => a + (st.totalTokens ?? 0), 0),
      0,
    );
    ...
    const nextCursor =
      allRuns.length > startIdx + limit ? allRuns[startIdx + limit - 1]!.runId : null;
