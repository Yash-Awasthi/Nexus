...
/**
 * Drift routes — Adaptive sampling parameter optimisation + EMA feedback loop.
 *
 * POST /drift/compute   — detect context and compute optimal LLM sampling params
 * POST /drift/rate      — record a user rating (1–5) to update EMA deltas
 * GET  /drift/detect    — detect context type for a message without computing params
 *
 * EMA persistence
 * ───────────────
...
async function ensureEmaSchema(pool: PgPool): Promise<void> {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS drift_ema (
    ...
    try {
      ...
      const { rows } = await this.pool.query(
        `SELECT temperature, top_p, frequency_penalty, presence_penalty, samples
           FROM drift_ema WHERE context = $1`,
        [context],
      );
  ...
  async set(context: ContextType, delta: LearnedDelta): Promise<void> {
    try {
      ...
      await this.pool.query(
        `INSERT INTO drift_ema (context, temperature, top_p, frequency_penalty, presence_penalty, samples, updated_at)
...
export async function driftRoutes(app: FastifyInstance): Promise<void> {
  ...
  }>("/drift/compute", { preHandler: requireAuth }, async (request, reply) => {
  ...
  }>("/drift/rate", { preHandler: requireAuth }, async (request, reply) => {
  ...
  }>("/drift/detect", { preHandler: requireAuth }, async (request, reply) => {
