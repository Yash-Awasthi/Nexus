/**
 * @nexus/api — GhostStack orchestration routes
 *
 * Exposes the GhostStack multi-agent runtime as a REST API:
...
async function _getOrchestrator(): Promise<ReturnType<GSModule["GhostStackOrchestrator"]["create"]>> {
  ...
  try {
    const gs = _require("@nexus/ghoststack") as GSModule;
...
export async function ghoststackRoutes(app: FastifyInstance) {
