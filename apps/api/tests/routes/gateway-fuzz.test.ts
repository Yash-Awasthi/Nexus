...
describe("POST /api/v1/gateway/messages — property-based fuzzing", () => {
   ...
   */
  test.prop([unknownModelArb, messagesArb], { numRuns: 25 })(
   ...
   */
  test.prop([messagesArb], { numRuns: 20 })(
    "nexus/fast with no GROQ key → 400 provider_unavailable",
    async (messages) => {
      delete process.env.GROQ_API_KEY;
      const res = await app.inject({
        method: "POST",
   ...
   */
  test.prop([messagesArb, optTemperature, optMaxTokens], { numRuns: 20 })(
    "max_spend_usd:0 always triggers 402 spend cap",
   ...
   */
  test.prop([messagesArb, optTemperature, optMaxTokens, optSystem], { numRuns: 20 })(
    "valid request with mocked Groq returns 200 with Anthropic message shape",
