...
const s = {
  ...
  entryHeader: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8 },
  ...
  tag: {
    fontSize: 11, background: "#1e1b4b", color: "#a5b4fc", padding: "2px 8px",
...
export default function Memory() {
  ...
  const load = (q?: string) => {
    ...
    api
      ...
      .catch(() =>
        setEntries([
          { id: "m1", content: "User prefers TypeScript with strict mode enabled.", category: "preference", tags: ["typescript", "coding"], createdAt: new Date().toISOString() },
          { id: "m2", content: "Working on @nexus/* multi-agent platform with 23 infrastructure gaps to close.", category: "project", tags: ["nexus", "infra"], createdAt: new Date().toISOString() },
          { id: "m3", content: "Graduating 2026-07-02 from NIT Raipur (CSE, CGPA 9.24).", category: "fact", tags: ["education"], createdAt: new Date().toISOString() },
        ]),
      )
  ...
  return (
    <div>
      ...
      <div style={s.searchRow}>
        ...
        />
        <button style={s.searchBtn} onClick={() => load(query)}>Search</button>
        {query && (
          <button style={{ ...s.searchBtn, background: "#334155" }} onClick={() => { setQuery(""); load(); }}>
            Clear
          </button>
        )}
      </div>
        ...
        entries.map((entry) => (
          <div key={entry.id} style={s.entry}>
            <div style={s.entryHeader}>
              ...
              <button style={s.deleteBtn} onClick={() => handleDelete(entry.id)} title="Forget">✕</button>
            </div>
          </div>
        ))
      )}
