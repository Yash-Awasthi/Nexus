...
const s = {
  ...
  type: { fontSize: 11, color: "#64748b", textTransform: "uppercase" as const, letterSpacing: "0.08em" },
  ...
  toggle: (enabled: boolean): React.CSSProperties => ({
    ...
    background: enabled ? "#7c3aed" : "#334155", position: "relative", flexShrink: 0,
...
export default function Connectors() {
  ...
  useEffect(() => {
    api
      ...
      .catch(() =>
        setConnectors([
          { id: "gmail", name: "Gmail", type: "email", enabled: true, status: "ok", lastSyncAt: new Date().toISOString() },
          { id: "github", name: "GitHub", type: "vcs", enabled: true, status: "ok", lastSyncAt: new Date().toISOString() },
          ...
          { id: "linear", name: "Linear", type: "issues", enabled: false, status: "error", error: "Invalid API token" },
  ...
  const toggle = (id: string) => {
    ...
    api.patch(`/connectors/${id}`, { enabled: !connectors.find((c) => c.id === id)?.enabled }).catch(() => {});
  };
    ...
    <div>
      <div style={s.header}>
        <div>
          ...
          <p style={{ color: "#64748b", margin: "4px 0 0" }}>Manage integrations and data sources</p>
        </div>
        <button style={s.addBtn}>+ Add Connector</button>
      </div>

      {error && (
        <div style={{ background: "#450a0a", border: "1px solid #7f1d1d", borderRadius: 8, padding: 14, marginBottom: 20, color: "#fca5a5", fontSize: 13 }}>
          {error}
        </div>
      )}
            ...
            <div key={c.id} style={s.card}>
              <div style={s.cardHeader}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <button style={s.toggle(c.enabled)} onClick={() => toggle(c.id)} title={c.enabled ? "Disable" : "Enable"} />
                </div>
              </div>
              {c.lastSyncAt && (
