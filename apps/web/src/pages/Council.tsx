  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, marginBottom: 4 }}>
        ...
        <span style={badge(PRIORITY_COLOR[signal.priority] ?? "#475569")}>
          {signal.priority}
        </span>
      </div>
  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
        <span style={badge(DECISION_COLOR[verdict.decision] ?? "#475569")}>
          {verdict.decision}
        </span>
      ...
      <p style={{ fontSize: 12, color: "#94a3b8", margin: 0, lineHeight: 1.45 }}>
        {verdict.rationale.slice(0, 140)}{verdict.rationale.length > 140 ? "…" : ""}
      </p>
      {verdict.costUsd && (
...
export default function Council() {
  ...
  async function deliberate() {
    ...
    setResult(null);
    try {
      const res = await api.post<{ ok: boolean; result: CouncilResult }>(
        "/council/deliberate",
        { proposal: { title, description: desc || undefined }, budgetUsd: parseFloat(budget) },
      );
  ...
  async function triggerBySignal() {
    ...
    try {
      const res = await api.post<{ ok: boolean; result: CouncilResult }>(
        "/council/trigger",
        { signalId: triggerSid, budgetUsd: parseFloat(budget) },
      );
      setResult(res.result);
      void fetchVerdicts(0, true);
      setVerdictOffset(0);
  ...
  return (
    <div style={{ padding: "0 4px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1.3fr", gap: 14 }}>
        ...
        <div style={panel}>
          <div style={sectionTitle}>Active Signals <span style={{ color: "#334155" }}>· live</span></div>

          {signalsError && (
            <div style={{ fontSize: 12, color: "#fca5a5" }}>{signalsError}</div>
          )}
        ...
        <div style={panel}>
          ...
          {result && (
            ...
            >
              ...
              <div style={{ fontSize: 11, color: "#475569", textTransform: "uppercase", marginBottom: 6 }}>
                Archetype votes
              </div>
                ...
                return (
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                      <span style={{ fontSize: 12, color: "#c4b5fd", fontWeight: 600 }}>{v.model}</span>
                      <span style={{ fontSize: 11, color: vc, fontWeight: 700, textTransform: "uppercase" }}>
                        {v.vote}
                      </span>
                    </div>
