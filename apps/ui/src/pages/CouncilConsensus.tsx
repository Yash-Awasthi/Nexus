  ...
  votes: [
    {
      ...
      provider: "anthropic",
      answer:
        "REST for public endpoints — simpler for third-party integration, better caching, widely supported.",
      confidence: 0.85,
      reasoning:
        "REST has broader ecosystem support, better HTTP caching semantics, and lower barrier for API consumers.",
      latencyMs: 820,
      tokens: 1240,
    },
    {
      ...
      confidence: 0.78,
      reasoning:
        "GraphQL excels when clients need flexible data fetching. The schema serves as self-documenting API contract.",
      latencyMs: 650,
      tokens: 1180,
    },
    {
      ...
      confidence: 0.92,
      reasoning:
        "A hybrid approach gives REST for simple operations and GraphQL for complex data needs. Slightly more maintenance but optimal flexibility.",
      latencyMs: 1100,
      tokens: 1350,
    },
    {
      ...
      provider: "deepseek",
      answer:
        "REST — explicit, cacheable, mature tooling. GraphQL adds unnecessary complexity for a public API.",
      confidence: 0.81,
      reasoning:
        "For a public API, simplicity and tooling maturity matter most. REST with OpenAPI spec provides excellent developer experience.",
      latencyMs: 2100,
      tokens: 1500,
    },
  ],
  consensusAnswer:
    "Hybrid approach: REST for simple CRUD operations with OpenAPI documentation, GraphQL for complex nested queries. This gives third-party developers simple endpoints while enabling efficient data fetching for complex use cases.",
  agreement: 0.62,
  rounds: 3,
  status: "converged",
            ...
            {votes.map((v) => (
              <th
                ...
                style={{
                  ...
                  color: getModelStyle(v.model).color,
                  fontWeight: 600,
                  textAlign: "center",
                }}
              >
                ...
                <td
                  ...
                  style={{
                    ...
                    background:
                      i === j
                        ? "transparent"
                        : `rgba(${val > 0.5 ? "34,197,94" : "239,68,68"},${val * 0.3})`,
                    fontWeight: 700,
                  }}
                >
    ...
    >
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        <p style={{ color: "#6b7280", marginBottom: "2rem" }}>
          Multi-model deliberation — {result.rounds} rounds, {result.votes.length} models
        </p>
        ...
        >
          ...
          >
            <span>
              Agreement:{" "}
              <strong style={{ color: result.agreement >= 0.6 ? "#22c55e" : "#f97316" }}>
            ...
            <span>
              Status:{" "}
              <strong style={{ color: result.status === "converged" ? "#22c55e" : "#f97316" }}>
                {result.status}
              </strong>
            </span>
          </div>
          ...
          {result.votes.map((vote, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: "0.7rem", color: "#6b7280" }}>
                      {vote.provider} • {vote.latencyMs}ms • {vote.tokens} tok
                    </div>
                  </div>
                  <div style={{ fontSize: "1.2rem" }}>
                    {i ===
                    result.votes.reduce(
                      (best, v, j) => (v.confidence > result.votes[best].confidence ? j : best),
                      0,
                    )
                      ? "👑"
                      : ""}
                ...
                <div style={{ fontSize: "0.85rem", lineHeight: 1.5, marginBottom: 8 }}>
                  {vote.answer}
                </div>
