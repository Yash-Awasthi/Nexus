...
export function loadSession(): CodegenSession | null {
  try {
    ...
    if (Date.now() - s.timestamp > MAX_AGE_MS) { localStorage.removeItem(KEY); return null }
    return s
  } catch { return null }
}
  ...
  return (
    ...
    >
      <div style={{ flex: 1, minWidth: 0 }}>
        <span style={{ fontSize: 11, color: '#3b82f6', fontWeight: 600 }}>Continue last session</span>
        <span style={{ fontSize: 11, color: '#555', marginLeft: 8 }}>{timeAgo(session.timestamp)}</span>
      ...
      <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
        <button
          ...
          style={{
            ...
            border: '1px solid #333', borderRadius: 5, padding: '5px 12px', cursor: 'pointer',
          }}
        >
          Start fresh
        </button>
        <button
          ...
          style={{
            ...
            border: 'none', borderRadius: 5, padding: '5px 14px', cursor: 'pointer', fontWeight: 500,
          }}
        >
          Continue →
        </button>
