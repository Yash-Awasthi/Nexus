  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 12, color: '#888' }}>
        <span style={{ color: accepted > 0 ? '#22c55e' : '#666', fontWeight: 600 }}>{accepted}</span>
      ...
      <div style={{ display: 'flex', gap: 6, marginLeft: 'auto', flexWrap: 'wrap' }}>
        {rollbackId && (
          <button
            onClick={() => { onRollback(); showToast('Rolled back') }}
            style={{
              fontSize: 11, color: '#f59e0b', background: 'none',
              border: '1px solid #78350f', borderRadius: 4, padding: '4px 10px', cursor: 'pointer',
            }}
          >
            ↩ Rollback
          </button>
        ...
        <button
          onClick={() => { onRejectAll(); showToast('All rejected') }}
          style={{
        ...
        <button
          onClick={() => { if (accepted > 0) { onApply(); showToast('Applied!') } }}
      ...
      {toast && (
        <div style={{
          ...
          padding: '8px 16px', fontSize: 13, color: '#e5e7eb',
