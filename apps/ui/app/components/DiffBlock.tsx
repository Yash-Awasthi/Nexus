...
const LINE_COLOR: Record<HunkLine['type'], string> = {
  added: '#86efac', removed: '#fca5a5', context: '#9ca3af',
}
          ...
          {(hovered || hunk.status === 'pending') && hunk.status !== 'rejected' && (
            <>
              {hunk.status === 'pending' && (
                <button
                  onClick={() => startEdit()}
                  style={{ fontSize: 11, color: '#888', background: 'none', border: '1px solid #333', borderRadius: 4, padding: '2px 8px', cursor: 'pointer' }}
                >
                  Edit
                </button>
              )}
              <button
                ...
                style={{ fontSize: 11, color: '#ef4444', background: 'none', border: '1px solid #7f1d1d', borderRadius: 4, padding: '2px 8px', cursor: 'pointer' }}
              >
                Reject
              </button>
              <button
                onClick={() => onAccept(hunk.id)}
                style={{ fontSize: 11, color: '#22c55e', background: 'none', border: '1px solid #14532d', borderRadius: 4, padding: '2px 10px', cursor: 'pointer', fontWeight: 600 }}
              >
                Accept
              </button>
            </>
      ...
      {!editing ? (
        <div style={{ fontFamily: 'monospace', fontSize: 12, overflowX: 'auto' }}>
          {hunk.lines.map((line, i) => (
            ...
            >
              <span style={{ color: '#444', minWidth: 36, textAlign: 'right', padding: '0 8px', userSelect: 'none', fontSize: 10 }}>
                {line.oldLine ?? ''}
              </span>
              <span style={{ color: '#444', minWidth: 36, textAlign: 'right', padding: '0 8px', userSelect: 'none', fontSize: 10 }}>
                {line.newLine ?? ''}
              </span>
              <span style={{ color: LINE_COLOR[line.type], padding: '0 4px 0 2px', userSelect: 'none' }}>
        ...
        <div style={{ padding: 8 }}>
          <textarea
            ...
            style={{
              ...
              color: '#e5e7eb', borderRadius: 4, padding: 8, fontFamily: 'monospace',
              fontSize: 12, resize: 'vertical', minHeight: 80, outline: 'none',
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
            <button
              onClick={submitEdit}
              style={{ fontSize: 11, color: '#22c55e', background: 'none', border: '1px solid #14532d', borderRadius: 4, padding: '3px 10px', cursor: 'pointer' }}
            >
              Apply edit
            </button>
            <button
              onClick={() => setEditing(false)}
              style={{ fontSize: 11, color: '#888', background: 'none', border: '1px solid #333', borderRadius: 4, padding: '3px 10px', cursor: 'pointer' }}
            >
              Cancel
            </button>
          </div>
