...
async function fetchResults(type: MentionType | 'all', query: string): Promise<SearchResult[]> {
  ...
  if (type === 'symbol') {
    try {
      ...
      if (r.ok) {
        const data: Array<{ name: string; type: string; file: string; line: number }> = await r.json()
        return data.slice(0, 8).map((s) => ({ label: s.name, value: s.name, sub: `${s.type} · ${s.file}:${s.line}` }))
      }
    } catch {}
  }
  if (type === 'web') {
...
export function ContextMention({
  ...
  useEffect(() => {
    if (!mention.isOpen) { setResults([]); return }
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      ...
      {mentions.length > 0 && (
        ...
        >
          {mentions.map((m, i) => (
            <ContextPill key={i} type={m.type} label={m.label} value={m.value} onRemove={() => removeMention(i)} />
          ))}
        </div>
      )}
      ...
      {mentions.length > 0 && (
        <div style={{ fontSize: 11, color: '#555', textAlign: 'right', marginTop: 3 }}>
          ~{estimatedTokens > 999 ? `${(estimatedTokens / 1000).toFixed(1)}k` : estimatedTokens} ctx tokens
        </div>
      )}
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {results.length === 0 ? (
              ...
              results.map((r, i) => (
                ...
                >
                  <div style={{ fontSize: 13, color: '#e5e7eb', marginBottom: r.sub ? 2 : 0 }}>{r.label}</div>
                  {r.sub && (
