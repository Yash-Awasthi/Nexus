...
function myersDiff(a: string[], b: string[]): Array<{ type: 'equal' | 'insert' | 'delete'; value: string }> {
      ...
      if (x >= n && y >= m) {
        ...
        for (let bd = d; bd > 0; bd--) {
          ...
          if (pk === -bd || (pk !== bd && (prev[pk - 1] ?? 0) < (prev[pk + 1] ?? 0))) {
            prevX = prev[pk + 1] ?? 0; prevY = prevX - (pk + 1)
          } else {
            prevX = (prev[pk - 1] ?? 0) + 1; prevY = prevX - (pk - 1)
          }
          while (cx > prevX && cy > prevY) { cx--; cy--; result.unshift({ type: 'equal', value: a[cx] }) }
          if (cx > prevX) { cx--; result.unshift({ type: 'delete', value: a[cx] }) }
          else { cy--; result.unshift({ type: 'insert', value: b[cy] }) }
        }
        while (cx > 0 && cy > 0) { cx--; cy--; result.unshift({ type: 'equal', value: a[cx] }) }
        return result
      }
    }
  }
...
export function DiffViewer({ filename, original, modified, onApply }: DiffViewerProps) {
  ...
  const handleEdit = useCallback((id: string, editedLines: HunkLine[]) => {
    setHunks(prev => prev.map(h => h.id === id ? { ...h, lines: editedLines, status: 'accepted' } : h))
  }, [])
