...
interface DailyPoint7 { day: string; count: number; }
interface DailyPoint30 { date: string; requests: number; cost: number; }
interface ProviderSlice { name: string; value: number; color: string; }
interface ModelRow { name: string; provider: string; requests: number; tokens: string; cost: string; }
  ...
  useEffect(() => {
    ...
    fetch("/api/analytics/providers")
      ...
      .then((data) => {
        ...
        if (arr.length > 0) {
          setProviders(arr.map((p: any, i: number) => ({
    ...
    fetch("/api/analytics/models?limit=5")
      ...
      .then((data) => {
        ...
        if (arr.length > 0) {
          setTopModels(arr.map((m: any) => ({
            name:     m.name ?? m.model ?? "",
            provider: m.provider ?? "",
            requests: m.requests ?? m.count ?? 0,
            tokens:   m.tokensFormatted ?? (typeof m.tokens === "number"
              ? (m.tokens >= 1_000_000 ? `${(m.tokens / 1_000_000).toFixed(1)}M` : `${Math.round(m.tokens / 1000)}K`)
              : (m.tokens ?? "—")),
            cost: m.costFormatted ?? (typeof m.costUsd === "number" ? `$${m.costUsd.toFixed(2)}` : (m.cost ?? "—")),
          })));
        } else { setTopModels(FALLBACK_MODELS); }
      })
            ...
            <>
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={chartData30} margin={{ top: 4, right: 8, bottom: 0, left: -20 }}>
                  <CartesianGrid {...GRID_PROPS} />
                  <XAxis dataKey="date" {...AXIS_PROPS} tickFormatter={xAxisTickFormatter30} interval={0} />
