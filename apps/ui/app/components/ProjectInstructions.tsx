...
function instrKey(id: string) {
  return `project_instructions_${id}`;
}

function load(projectId: string): string {
  try {
    return localStorage.getItem(instrKey(projectId)) ?? "";
  } catch {
    return "";
  }
}
...
function save(projectId: string, value: string) {
  try {
    localStorage.setItem(instrKey(projectId), value);
  } catch {}
}
  ...
  useEffect(() => {
    fetch("/api/stm")
      ...
      .then((data) => {
        ...
        setModules(mods);
        setActiveModules(
          new Set(mods.filter((m: STMModule) => m.active).map((m: STMModule) => m.id)),
        );
      })
      .catch(() => {});
  }, []);
