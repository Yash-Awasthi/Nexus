      ...
      <Card>
        <CardContent className="pt-4 space-y-3">
          <p className="text-sm text-muted-foreground">
            Paste a conversation to analyze for echo chamber patterns — excessive agreement,
            sycophancy, groupthink, or lack of critical pushback.
          </p>
          <Textarea
            rows={8}
            placeholder={
          ...
          <CardContent className="pt-4 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`font-semibold ${result.detected ? "text-orange-600" : "text-green-600 dark:text-green-400"}`}
                >
                ...
                {result.patterns.map((p, i) => (
                  <div key={i} className="flex items-start gap-2 text-sm">
                    <Badge
                      className={
                        p.severity === "high"
                          ? "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400 shrink-0"
                          : p.severity === "medium"
    ...
    <Card>
      <CardContent className="pt-4 space-y-4">
        <div className="flex items-center justify-between">
          ...
          >
            <span
              className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${edit.enabled ? "translate-x-6" : "translate-x-0.5"}`}
            />
          </button>
        </div>
        <div className="space-y-1">
          <label className="text-sm font-medium flex justify-between">
            ...
            <span className="font-normal text-muted-foreground">
              {Math.round((edit.threshold ?? 0.6) * 100)}%
            </span>
          </label>
          <input
            type="range"
            ...
            onChange={(e) =>
              setEdit((prev) => ({ ...prev, threshold: parseFloat(e.target.value) }))
            }
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() =>
              setEdit((prev) => ({ ...prev, autoInjectDissent: !prev.autoInjectDissent }))
            }
          ...
          >
            <span
              className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${edit.autoInjectDissent ? "translate-x-6" : "translate-x-0.5"}`}
            />
          </button>
        </div>
        <div className="space-y-1">
          ...
          <select
            ...
            onChange={(e) =>
              setEdit((prev) => ({
                ...prev,
                aggressiveness: e.target.value as EchoConfig["aggressiveness"],
              }))
            }
          >
            {["gentle", "moderate", "strong"].map((a) => (
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <Tabs defaultValue="detect">
        ...
        <TabsContent value="dissent" className="mt-4">
          <DissentTab />
        </TabsContent>
