...
export default function VideoTranscript() {
  ...
  const transcribe = useCallback(async (sourceUrl?: string, base64?: string) => {
    const target = sourceUrl ?? url.trim();
    if (!target && !base64) return;
    setTranscribing(true); setErr(""); setResult(null);
  ...
  const downloadSRT = () => {
    ...
    const srt = result.segments.map((seg, i) => {
      ...
      const end = seg.endSec ? formatTime(seg.endSec).replace(":", ",") : formatTime(seg.startSec + 5).replace(":", ",");
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      {sources.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {sources.filter(s => s.supported).map(s => (
        ...
        <CardContent className="pt-4 space-y-3">
          <div className="flex gap-2">
            ...
            <Button onClick={() => transcribe()} disabled={transcribing || !url.trim()}>
              {transcribing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
            </Button>
          </div>
          ...
          <input ref={fileRef} type="file" accept="video/*,audio/*" className="hidden" onChange={e => e.target.files?.[0] && handleFile(e.target.files[0])} />
          <Button variant="outline" className="w-full" onClick={() => fileRef.current?.click()} disabled={transcribing}>
            <Upload className="w-4 h-4 mr-2" />Upload video/audio file
          </Button>
            ...
            <div className="flex items-center gap-2">
              {result.duration && (
                <Badge variant="outline" className="flex items-center gap-1">
                  <Clock className="w-3 h-3" />{formatTime(result.duration)}
                </Badge>
              )}
          ...
          <div className="flex gap-2">
            {(["segments", "text", "chapters"] as const).map(m => (
              result.chapters || m !== "chapters" ? (
                <Button key={m} size="sm" variant={viewMode === m ? "default" : "outline"} onClick={() => setViewMode(m)} className="capitalize">
                  {m}
                </Button>
              ) : null
            ))}
          ...
          {viewMode === "segments" && (
            <div className="space-y-1 max-h-[500px] overflow-y-auto">
              {result.segments.map((seg, i) => (
                <div key={i} className="flex items-start gap-3 py-1.5 px-2 rounded hover:bg-muted/30 group">
                  ...
                  {seg.speaker && (
                    <span className="text-xs font-medium text-primary shrink-0 w-20 truncate pt-0.5 flex items-center gap-0.5">
                      <User className="w-3 h-3" />{seg.speaker}
                    </span>
                  )}
          ...
          {viewMode === "text" && (
            <Card>
              <CardContent className="pt-4">
                <p className="text-sm whitespace-pre-wrap max-h-[500px] overflow-y-auto">{result.fullText}</p>
              </CardContent>
            </Card>
          )}
            ...
            <div className="space-y-1">
              {result.chapters.map((ch, i) => (
                <div key={i} className="flex items-center gap-3 py-2 border-b last:border-0">
                  <span className="text-xs text-muted-foreground font-mono w-12">{formatTime(ch.startSec)}</span>
