...
import {
  Search, Send, Loader2, X, CheckCircle2, AlertCircle,
  FileText, ChevronDown, ChevronRight, BookOpen, BarChart3,
} from "lucide-react";
...
function usePastJobs() {
  const [jobs, setJobs] = useState<Array<{ id: string; query: string; status: string; createdAt: string }>>([]);
      ...
      >
        ...
        <ScrollArea className="flex-1">
          <div className="p-2 space-y-1">
            {pastJobs.length === 0 ? (
              <p className="text-xs text-muted-foreground px-2 py-3 text-center">No past research</p>
            ) : (
              pastJobs.map((j) => (
                <button
                  key={j.id}
                ...
                >
                  <div className="flex items-center gap-1.5 mb-1">
                    <span className={`text-[10px] ${j.status === "done" ? "text-green-400" : j.status === "failed" ? "text-destructive" : "text-yellow-400"}`}>
                      ● {j.status}
                    </span>
                  </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          <Textarea
            ...
            onKeyDown={(e) => { if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) { (e.target as HTMLFormElement).form?.requestSubmit(); }}}
          />
          <Button
            type="submit"
                    ...
                    return (
                      <div key={step.id}>
                        ...
                        {expanded && step.detail && (
                          <p className="ml-8 text-xs text-muted-foreground py-1 pr-2">{step.detail}</p>
                        )}
                      </div>
                    );
                  })}
              ...
              {job.citations.length > 0 && (
                <div>
                  ...
                  >
                    ...
                    {expandCitations ? <ChevronDown className="size-3" /> : <ChevronRight className="size-3" />}
                  </button>
                  {expandCitations && (
                      ...
                      {job.citations.map((c) => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <Badge variant="outline" className="text-[10px] shrink-0">Cycle {c.cycleIndex + 1}</Badge>
                          </div>
                          {c.url && (
                            <a
                              href={c.url}
            ...
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center p-10">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-2 max-w-sm">
                  Multi-step agentic research with web search, reasoning cycles,
                  and cited synthesis. Enter a research question above.
                </p>
              </div>
              <div className="flex flex-wrap gap-2 justify-center text-xs text-muted-foreground">
                {["Clarification", "Planning", "Research cycles", "Synthesis", "Citations"].map((s) => (
