...
function RunTab({ schemas, templates }: { schemas: ExtractionSchema[]; templates: Template[] }) {
  ...
  const runPreview = useCallback(async () => {
    ...
    if (useCustom && customSchema.trim()) {
      try { body.schema = JSON.parse(customSchema); } catch { setErr("Invalid JSON schema"); setPreviewing(false); return; }
  ...
  const run = useCallback(async () => {
    ...
    const body: Record<string, unknown> = { text: text.trim() };
    if (useCustom && customSchema.trim()) {
      try { body.schema = JSON.parse(customSchema); } catch { setErr("Invalid JSON schema"); setRunning(false); return; }
    } else if (schemaId) body.schemaId = schemaId;
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2 flex-wrap">
            <Button variant="outline" size="sm" onClick={inferSchema} disabled={inferring || !text.trim()}>
              {inferring ? <Loader2 className="w-3 h-3 animate-spin mr-1" /> : <Wand2 className="w-3 h-3 mr-1" />}
              Infer schema
            </Button>
            <Button variant="outline" size="sm" onClick={runPreview} disabled={previewing || !text.trim()}>
            ...
            <Button size="sm" onClick={run} disabled={running || !text.trim()}>
              {running ? <><Loader2 className="w-4 h-4 animate-spin mr-1" />Extracting…</> : <><Braces className="w-4 h-4 mr-1" />Extract</>}
            </Button>
          </div>
        ...
        <div className="space-y-3">
          {preview && (
            <div>
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">Preview (not saved)</p>
              <pre className="text-xs bg-muted p-3 rounded-md overflow-auto max-h-40">{JSON.stringify(preview, null, 2)}</pre>
            </div>
          )}
          {result && (
            <div>
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">Extraction Result</p>
              <pre className="text-xs bg-muted p-3 rounded-md overflow-auto max-h-64 whitespace-pre-wrap">{JSON.stringify(result, null, 2)}</pre>
              <Button
                variant="outline"
                size="sm"
                className="mt-2"
...
function SchemasTab({ schemas, onRefresh }: { schemas: ExtractionSchema[]; onRefresh: () => void }) {
  ...
  const create = useCallback(async () => {
    ...
    let parsedSchema;
    try { parsedSchema = JSON.parse(newSchema); } catch { setErr("Invalid JSON"); return; }
    ...
    if (r?.ok) { setCreating(false); setNewName(""); setNewSchema("{}"); onRefresh(); }
      ...
      {schemas.length === 0 && !creating ? (
        <Card>
          <CardContent className="pt-8 pb-8 text-center space-y-3">
            ...
            <Button size="sm" onClick={() => setCreating(true)}>Add schema</Button>
          </CardContent>
        </Card>
      ) : (
        <>
              ...
              <CardContent className="pt-3 pb-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <pre className="text-xs text-muted-foreground mt-1 truncate">{JSON.stringify(s.schema)}</pre>
                  </div>
                  <Button size="icon" variant="ghost" className="h-7 w-7 text-red-400 shrink-0" onClick={() => del(s.id)} disabled={deleting === s.id}>
          ...
          <Button size="sm" variant="outline" onClick={() => setCreating(!creating)}>
            <Plus className="w-4 h-4 mr-1" />{creating ? "Cancel" : "New schema"}
          </Button>
        </>
      )}
      ...
      {creating && (
        <Card>
          <CardContent className="pt-4 space-y-3">
            <Input placeholder="Schema name" value={newName} onChange={e => setNewName(e.target.value)} />
            <Textarea
              rows={6}
              placeholder={'{\n  "company": { "type": "string" },\n  "revenue": { "type": "number" }\n}'}
              value={newSchema}
...
function JobsTab() {
  ...
  useEffect(() => {
    fetch("/api/extraction/jobs").then(r => r.ok ? r.json() : { jobs: [] }).then(d => {
      setJobs(d.jobs ?? d);
  ...
  const exportJob = async (id: string) => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `extraction-${id}.json`; a.click();
    URL.revokeObjectURL(url);
  };
  ...
  if (!jobs.length) return <Card><CardContent className="pt-8 pb-8 text-center text-muted-foreground">No extraction jobs yet</CardContent></Card>;
        ...
        <Card key={j.id}>
          <CardContent className="pt-3 pb-3">
            <div className="flex items-center gap-3">
              {j.status === "done" ? <CheckCircle className="w-4 h-4 text-green-500 shrink-0" />
                : j.status === "failed" ? <XCircle className="w-4 h-4 text-red-500 shrink-0" />
                : <Loader2 className="w-4 h-4 animate-spin text-muted-foreground shrink-0" />}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium">{j.schemaName ?? "Custom schema"}</span>
                  <Badge variant={j.status === "done" ? "default" : j.status === "failed" ? "destructive" : "secondary"} className="text-xs">
                    {j.status}
                  </Badge>
                </div>
                <p className="text-xs text-muted-foreground">{new Date(j.createdAt).toLocaleString()}{j.durationMs ? ` · ${j.durationMs}ms` : ""}</p>
              </div>
              <div className="flex gap-1 shrink-0">
                {j.status === "done" && (
                  <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => exportJob(j.id)}>
                    <Download className="w-3 h-3" />
                  </Button>
                )}
                <Button size="icon" variant="ghost" className="h-7 w-7 text-red-400" onClick={() => deleteJob(j.id)}>
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <Tabs defaultValue="run">
        <TabsList>
          <TabsTrigger value="run"><Play className="w-4 h-4 mr-1" />Extract</TabsTrigger>
          <TabsTrigger value="schemas"><FileJson className="w-4 h-4 mr-1" />Schemas</TabsTrigger>
          <TabsTrigger value="jobs"><History className="w-4 h-4 mr-1" />Job History</TabsTrigger>
        </TabsList>
