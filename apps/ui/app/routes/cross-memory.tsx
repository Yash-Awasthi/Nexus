...
function RetrieveTab() {
  ...
  const retrieve = useCallback(async () => {
    ...
    if (agentIds.trim()) body.agentIds = agentIds.split(",").map(s => s.trim()).filter(Boolean);
        ...
        <CardContent className="pt-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs text-muted-foreground">Agent IDs (comma-separated, optional)</label>
              <Input placeholder="agent-1, agent-2…" value={agentIds} onChange={e => setAgentIds(e.target.value)} />
            </div>
            <div className="space-y-1">
          ...
          <Button onClick={retrieve} disabled={loading || !query.trim()}>
            {loading ? <><Loader2 className="w-4 h-4 animate-spin mr-2" />Retrieving…</> : <><Search className="w-4 h-4 mr-2" />Retrieve</>}
          </Button>
        </CardContent>
      </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              {result.hits.length} result(s){result.total !== undefined ? ` of ${result.total}` : ""}
            </CardTitle>
          </CardHeader>
            ...
            {result.hits.map((hit, i) => (
              <div key={i} className="p-3 rounded-lg bg-muted/50 space-y-1">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  {hit.score !== undefined && (
                    <Badge variant="outline" className="ml-auto">{(hit.score * 100).toFixed(1)}%</Badge>
                  )}
                </div>
      ...
      <Card>
        <CardContent className="pt-4 space-y-3">
          ...
          <Button onClick={build} disabled={loading || !query.trim()}>
            {loading ? <><Loader2 className="w-4 h-4 animate-spin mr-2" />Building…</> : <><Layers className="w-4 h-4 mr-2" />Build Context</>}
          </Button>
        </CardContent>
      </Card>
          ...
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {result.sources !== undefined && <Badge variant="secondary">{result.sources} sources</Badge>}
                {result.tokens !== undefined && <Badge variant="outline">{result.tokens} tokens</Badge>}
              </div>
            </CardTitle>
          </CardHeader>
          <CardContent>
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <Tabs defaultValue="retrieve">
        <TabsList>
          <TabsTrigger value="retrieve"><Search className="w-4 h-4 mr-1" />Retrieve</TabsTrigger>
          <TabsTrigger value="context"><Layers className="w-4 h-4 mr-1" />Context</TabsTrigger>
        </TabsList>
        <TabsContent value="retrieve" className="mt-4"><RetrieveTab /></TabsContent>
        <TabsContent value="context" className="mt-4"><ContextTab /></TabsContent>
      </Tabs>
    </div>
  );
}
