...
export default function Usage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [q, u] = await Promise.allSettled([
        authFetch("/api/v1/billing/quota").then((r) => (r.ok ? (r.json() as Promise<Quota>) : null)),
  ...
  const byModel = (usage?.byModelDay ?? []).reduce<
    Record<string, { promptTokens: number; completionTokens: number; costUsd: number; requests: number }>
  >((acc, r) => {
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <Card>
              <CardContent className="pt-4">
                ...
                <p className="text-2xl font-bold">{capUsd !== null ? fmtUsd(capUsd) : "Unlimited"}</p>
              </CardContent>
            </Card>
            <Card
              className={
