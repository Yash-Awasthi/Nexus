    ...
    <div className="space-y-4">
      <Card>
        <CardContent className="pt-4 space-y-3">
          <Textarea
            rows={5}
            placeholder="Enter a prompt to check against filters…"
            value={prompt}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold">
                  {result.allowed ? "Prompt Allowed" : "Prompt Blocked"}
                </span>
              </div>
    ...
    <div className="space-y-4">
      <Card>
        <CardContent className="pt-4 space-y-3">
          <Textarea
            rows={5}
            placeholder="Enter a prompt with potential violations to sanitize…"
            value={prompt}
            onChange={(e) => setPrompt(e.target.value)}
            className="resize-none"
          />
    ...
    <div className="space-y-3">
          ...
          <Card key={item.id}>
            <CardContent className="pt-3 space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setItems((prev) =>
                    prev.map((i) => (i.id === item.id ? { ...i, prompt: e.target.value } : i)),
                  )
                }
      ...
      <div className="flex gap-2">
        <Button
          variant="outline"
          size="sm"
          onClick={() => setItems((prev) => [...prev, { id: String(Date.now()), prompt: "" }])}
        >
...
function PatternsTab() {
  ...
  useEffect(() => {
    fetch("/api/prompt-filter/patterns")
      ...
      {patterns.map((p) => (
        <Card key={p.id} className={!p.enabled ? "opacity-50" : ""}>
          <CardContent className="pt-3 pb-3">
            ...
            {p.pattern && (
              <code className="text-xs text-muted-foreground block mt-1 truncate">{p.pattern}</code>
            )}
          </CardContent>
        </Card>
      ))}
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <Tabs defaultValue="check">
        ...
        <TabsContent value="sanitize" className="mt-4">
          <SanitizeTab />
        </TabsContent>
