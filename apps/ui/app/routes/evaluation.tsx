...
const SEED_EVALS: EvalEntry[] = [
  { id: "ev_1", conversation: "API architecture discussion",  quality: 88, coherence: 0.91, consensus: 0.76, diversity: 0.93, date: "2026-04-22" },
  { id: "ev_2", conversation: "ML model selection",          quality: 79, coherence: 0.84, consensus: 0.68, diversity: 0.87, date: "2026-04-21" },
  { id: "ev_3", conversation: "Security review",             quality: 92, coherence: 0.95, consensus: 0.81, diversity: 0.79, date: "2026-04-21" },
  { id: "ev_4", conversation: "Database schema design",      quality: 55, coherence: 0.61, consensus: 0.49, diversity: 0.95, date: "2026-04-20" },
  { id: "ev_5", conversation: "CI/CD pipeline setup",        quality: 83, coherence: 0.88, consensus: 0.74, diversity: 0.91, date: "2026-04-20" },
  { id: "ev_6", conversation: "React state management",      quality: 71, coherence: 0.77, consensus: 0.65, diversity: 0.88, date: "2026-04-19" },
  { id: "ev_7", conversation: "Docker containerization",     quality: 90, coherence: 0.93, consensus: 0.79, diversity: 0.85, date: "2026-04-19" },
  { id: "ev_8", conversation: "GraphQL vs REST debate",      quality: 48, coherence: 0.55, consensus: 0.41, diversity: 0.98, date: "2026-04-18" },
];
...
export default function EvaluationPage() {
  ...
  const { avgQuality, avgCoherence, avgConsensus, avgDiversity } = useMemo(() => {
    ...
    if (len === 0) return { avgQuality: 0, avgCoherence: "0.00", avgConsensus: "0.00", avgDiversity: "0.00" };
    return {
  ...
  const handleRun = async () => {
    ...
    const topic = customTopic.trim() || sampleTopics[Math.floor(Math.random() * sampleTopics.length)];
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Measure and track quality metrics across AI council conversations</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button size="sm" className="gap-2" onClick={handleRun} disabled={isRunning}>
            {isRunning ? <><Loader2 className="size-3.5 animate-spin" />Running...</> : <><Play className="size-3.5" />Run Evaluation</>}
          </Button>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Average Quality", value: avgQuality + "%",    color: scoreColor(avgQuality, true) },
            ...
            <Card key={stat.label}>
              <CardHeader className="pb-1">
                <CardTitle className="text-xs text-muted-foreground font-normal">{stat.label}</CardTitle>
              </CardHeader>
              <CardContent>
                {loading
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground">Conversation</th>
                    <th className="text-center px-4 py-3 font-medium text-muted-foreground">Quality</th>
                    <th className="text-center px-4 py-3 font-medium text-muted-foreground">Coherence</th>
                    <th className="text-center px-4 py-3 font-medium text-muted-foreground">Consensus</th>
                    <th className="text-center px-4 py-3 font-medium text-muted-foreground">Diversity</th>
                    <th className="text-right px-4 py-3 font-medium text-muted-foreground">Date</th>
                  </tr>
                </thead>
                <tbody>
                  {evals.map((ev, i) => (
                    <tr key={ev.id} className={"border-b border-border last:border-0 hover:bg-muted/30 transition-colors " + (i % 2 === 0 ? "" : "bg-muted/10")}>
                      <td className="px-4 py-3 font-medium">{ev.conversation}</td>
                      <td className={"px-4 py-3 text-center font-mono font-semibold " + scoreColor(ev.quality, true)}>{ev.quality}%</td>
                      <td className={"px-4 py-3 text-center font-mono font-semibold " + scoreColor(ev.coherence)}>{ev.coherence.toFixed(2)}</td>
                      <td className={"px-4 py-3 text-center font-mono font-semibold " + scoreColor(ev.consensus)}>{ev.consensus.toFixed(2)}</td>
                      <td className={"px-4 py-3 text-center font-mono font-semibold " + scoreColor(ev.diversity)}>{ev.diversity.toFixed(2)}</td>
