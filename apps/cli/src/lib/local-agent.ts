...
function runCommand(command: string, root: string, maxOut: number, timeoutMs: number): Promise<string> {
...
export function buildLocalCodingTools(rootDir: string, enableShell = true): RuntimeToolSet {
  ...
  set.add({
    ...
    handler: async (args) =>
      clip(await fs.readFile(await safeResolve(root, String(args.path ?? "")), "utf8"), DEFAULT_MAX_OUTPUT),
  });
...
export function makeLocalLlm(provider: string, model?: string, apiKey?: string): LlmToolFn {
  ...
  if (!key) throw new Error(`missing_api_key (provider=${p}) — set the provider env var or pass --api-key`);
  const driver =
    p === "groq"
