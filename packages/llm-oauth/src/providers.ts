...
export class FetchTokenHttp implements TokenHttp {
  async postForm(url: string, params: Record<string, string>): Promise<unknown> {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `token endpoint ${res.status}: ${e.error ?? ""} ${e.error_description ?? ""}`.trim(),
      );
    }
    return json;
  }
...
export const DESCRIPTORS: ProviderDescriptor[] = [
  ...
  },
  {
    ...
    reason:
      "GitHub Models inference auth scope for third-party OAuth apps not clearly documented (preview).",
