...
the provider _officially_ supports for third-party apps (the operator's own
...
secret and the only asset that _must_ be encrypted at rest.
...
- **IV reuse under GCM is catastrophic** (nonce reuse breaks confidentiality _and_
...
- **Use a dedicated key, not `NEXUS_SECRETS_KEY`.** Although the wire format is
  interoperable, `secret-crypto.ts` documents _"each purpose (MFA, OAuth) uses its
  own key — no implicit fallback chain."_ Provision a separate var (e.g.
...
- **Hard rules for the caller / future routes:**
  - Never log `accessToken`, `refreshToken`, `client_secret`, PKCE verifier, or a
    sealed blob. Log an account id or a token _prefix_ only (mirror the BYOK
    `keyPrefix` pattern).
