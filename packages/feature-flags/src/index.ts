...
export class PollingFlagStore implements FlagStore {
  ...
  private async _poll(): Promise<void> {
    try {
      ...
      if (res.ok) {
        ...
        const safe = Object.create(null) as Record<string, FlagValue>;
  ...
  set(key: string, value: FlagValue): void {
    ...
    const safe: FlagValue =
      value !== null && typeof value === "object"
        ? (JSON.parse(JSON.stringify(value)) as FlagValue)
        : value;
    this.cache[key] = safe;
  }
...
export class FileFlagStore implements FlagStore {
  ...
  async load(): Promise<void> {
    try {
      ...
      const raw = JSON.parse(content) as Record<string, FlagValue>;
      const safe: Record<string, FlagValue> = {};
