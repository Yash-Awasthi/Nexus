...
export async function listMonitors(): Promise<Array<{
  id: string; name: string; url: string; status: string; uptime: number;
}>> {
...
export async function getMonitor(id: string): Promise<{
  name: string; status: string; uptime: number; url: string;
}> {
  const data = await uptime(`/monitors/${id}`) as {
    data: { attributes: { pronounceable_name: string; status: string; uptime: number; url: string } };
  };
  return {
...
export async function listIncidents(): Promise<Array<{
  id: string; name: string; status: string; started: string;
}>> {
  const data = await uptime('/incidents?per_page=20') as {
    data: Array<{ id: string; attributes: { name: string; status: string; started_at: string } }>;
  };
...
export async function createMonitor(
  name: string, url: string, checkFrequency = 180,
): Promise<{ id: string }> {
...
export async function sendLog(message: string, level: string, meta?: Record<string, unknown>): Promise<void> {
