...
export interface DriveDownloadTask {
  taskType: "drive.download";
  fileId: string;
}
export interface DriveListTask {
  taskType: "drive.list";
  folderId?: string;
  mimeType?: string;
  maxResults?: number;
}
export interface DriveCreateFolderTask {
  taskType: "drive.create-folder";
  name: string;
  parentId?: string;
}
...
async function execute(
...
): Promise<DriveFileResult | DriveListResult | { content: string }> {
  ...
  if (task.taskType === "drive.upload") {
    ...
    const metadata = {
      ...
      ...(task.parentId ? { parents: [task.parentId] } : {}),
    };
    ...
    if (!response.ok)
      throw new AdapterHttpError("nexus-adapter-drive", response.status, await response.text());
  ...
  if (task.taskType === "drive.download") {
    ...
    const response = await fetch(`${DRIVE_BASE}/files/${task.fileId}?alt=media`, {
...
export const driveAdapter = defineAdapter<
  DriveTask,
  DriveFileResult | DriveListResult | { content: string }
>({
  name: "nexus-adapter-drive",
  version: "0.1.0",
