...
export async function listProjects(): Promise<{ id: string; name: string }[]> {
  const data = (await dopplerFetch("/projects")) as { projects: { id: string; name: string }[] };
  return data.projects;
}

export async function listSecrets(project: string, config: string): Promise<string[]> {
  const data = (await dopplerFetch(
    `/configs/config/secrets/names?project=${project}&config=${config}`,
  )) as {
    names: string[];
  };
  return data.names;
...
export async function getSecret(project: string, config: string, name: string): Promise<string> {
  const data = (await dopplerFetch(
...
export async function deleteSecret(project: string, config: string, name: string): Promise<void> {
  await dopplerFetch(`/configs/config/secret?project=${project}&config=${config}&name=${name}`, {
    method: "DELETE",
  });
}
