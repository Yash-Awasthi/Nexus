...
export interface CloudflareDeployPagesTask { taskType: "cloudflare.deploy-pages"; projectName: string; branch?: string; }
export interface CloudflareR2PutTask { taskType: "cloudflare.r2-put"; bucketName: string; key: string; body: string; contentType?: string; }
export interface CloudflareR2GetTask { taskType: "cloudflare.r2-get"; bucketName: string; key: string; }
export interface CloudflarePurgeCacheTask { taskType: "cloudflare.purge-cache"; zoneId: string; urls?: string[]; purgeEverything?: boolean; }
export type CloudflareTask = CloudflareDeployPagesTask | CloudflareR2PutTask | CloudflareR2GetTask | CloudflarePurgeCacheTask;
export interface CloudflareResult { ok: boolean; result?: unknown; }
...
async function execute(task: CloudflareTask, ctx: IExecutionContext): Promise<CloudflareResult> {
  ...
  if (task.taskType === "cloudflare.deploy-pages") {
    ...
    const response = await cfFetch(`${CF_BASE}/accounts/${accountId}/pages/projects/${task.projectName}/deployments`, token, {
      method: "POST",
      body: JSON.stringify({ branch: task.branch ?? "main" }),
    });
    if (!response.ok) throw new AdapterHttpError("nexus-adapter-cloudflare", response.status, await response.text());
  ...
  if (task.taskType === "cloudflare.r2-put") {
    ...
    const response = await cfFetch(`${CF_BASE}/accounts/${accountId}/r2/buckets/${task.bucketName}/objects/${encodeURIComponent(task.key)}`, token, {
      method: "PUT",
  ...
  const response = await cfFetch(`${CF_BASE}/zones/${task.zoneId}/purge_cache`, token, { method: "POST", body: JSON.stringify(body) });
  if (!response.ok) throw new AdapterHttpError("nexus-adapter-cloudflare", response.status, await response.text());
  return { ok: true };
}

export const cloudflareAdapter = defineAdapter<CloudflareTask, CloudflareResult>({
  name: "nexus-adapter-cloudflare", version: "0.1.0", capabilities: ["deploy.trigger", "storage.read", "storage.write"],
  taskTypes: ["cloudflare.deploy-pages", "cloudflare.r2-put", "cloudflare.r2-get", "cloudflare.purge-cache"],
  execute,
});
