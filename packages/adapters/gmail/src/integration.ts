...
export async function listEmails(
  ...
  return Promise.all(ids.map(async (m) => {
...
export async function getEmail(id: string): Promise<{ subject: string; from: string; body: string }> {
  const gmail = gmailAuth();
...
export async function sendEmail(to: string, subject: string, body: string): Promise<string> {
  ...
  const res = await gmail.users.messages.send({
    userId: 'me', requestBody: { raw: encodeMessage(to, subject, body) },
  });
  return res.data.id ?? '';
}
...
export async function replyEmail(
  ...
  await gmail.users.messages.send({
    userId: 'me',
    requestBody: { raw: encodeMessage(to, subject, body, messageId), threadId: orig.data.threadId ?? undefined },
  });
}
...
export async function archiveEmail(messageId: string): Promise<void> {
  ...
  await gmail.users.messages.modify({
    userId: 'me', id: messageId, requestBody: { removeLabelIds: ['INBOX'] },
  });
}
