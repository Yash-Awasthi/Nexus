...
export interface VercelDeployTask { taskType: "vercel.deploy"; projectId: string; gitSource?: { ref: string; repoId?: string; type?: "github" | "gitlab" | "bitbucket" }; env?: Record<string, string>; }
export interface VercelListDeploymentsTask { taskType: "vercel.list-deployments"; projectId?: string; limit?: number; state?: "BUILDING" | "ERROR" | "INITIALIZING" | "QUEUED" | "READY" | "CANCELED"; }
export interface VercelAliasTask { taskType: "vercel.alias"; deploymentId: string; alias: string; }
export type VercelTask = VercelDeployTask | VercelListDeploymentsTask | VercelAliasTask;
export interface VercelDeploymentResult { id: string; url: string; state: string; createdAt: number; }
export interface VercelListResult { deployments: VercelDeploymentResult[]; }

async function execute(task: VercelTask, ctx: IExecutionContext): Promise<VercelDeploymentResult | VercelListResult | { ok: boolean; alias: string }> {
  ...
  if (task.taskType === "vercel.deploy") {
    ...
    const response = await fetch(`${VERCEL_BASE}/v13/deployments${teamParam}`, {
      ...
      body: JSON.stringify({ name: task.projectId, gitSource: task.gitSource, env: Object.entries(task.env ?? {}).map(([key, value]) => ({ key, value, type: "plain" })) }),
    });
    if (!response.ok) throw new AdapterHttpError("nexus-adapter-vercel", response.status, await response.text());
  ...
  const response = await fetch(`${VERCEL_BASE}/v2/deployments/${task.deploymentId}/aliases${teamParam}`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
...
export const vercelAdapter = defineAdapter<VercelTask, VercelDeploymentResult | VercelListResult | { ok: boolean; alias: string }>({
  name: "nexus-adapter-vercel", version: "0.1.0", capabilities: ["deploy.trigger"],
