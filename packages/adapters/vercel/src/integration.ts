...
export async function listProjects(): Promise<Array<{ id: string; name: string; url: string }>> {
  const data = await vercelFetch('/v9/projects?limit=50') as { projects: Array<{ id: string; name: string; link?: { deployHooks?: unknown[] } }> };
  return data.projects.map((p) => ({ id: p.id, name: p.name, url: `https://${p.name}.vercel.app` }));
}

export async function listDeployments(
  projectId?: string,
): Promise<Array<{ uid: string; url: string; state: string; created: number }>> {
  const query = projectId ? `?projectId=${projectId}&limit=10` : '?limit=10';
  const data = await vercelFetch(`/v6/deployments${query}`) as {
    deployments: Array<{ uid: string; url: string; state: string; created: number }>;
  };
...
export async function listEnvVars(
  projectId: string,
): Promise<Array<{ id: string; key: string; target: string[] }>> {
