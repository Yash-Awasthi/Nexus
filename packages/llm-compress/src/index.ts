...
export function resolveToolProfile(toolName: string): ToolProfileName {
  ...
  if (
    /build|compile|\btsc\b|test|vitest|jest|lint|\bnpm\b|pnpm|yarn|\bmake\b|cargo|gradle|webpack|bundle/.test(
      n,
    )
  )
    return "build-log";
...
export function injectSystemPrompt(base: string, injectors: readonly InjectorName[]): string {
  ...
  const blocks = injectors.map((n) => INJECTORS[n].text).filter((text) => !trimmed.includes(text));
...
function acceptStep(
  ...
  if (minGainPercent && minGainPercent > 0) {
    ...
    if (gain < minGainPercent)
      return { accept: false, note: `below-min-gain(${gain.toFixed(1)}%)` };
  }
  return { accept: true };
}
...
const CAVEMAN_RULES: readonly CavemanRule[] = [
  ...
  },
  {
    ...
    minIntensity: "lite",
    keyword:
      /basically|essentially|actually|really|very|quite|just|simply|literally|honestly|obviously|clearly|certainly|definitely/i,
    apply: (t) =>
      t.replace(
  ...
  },
  {
    ...
    minIntensity: "lite",
    keyword:
      /I think|I believe|I guess|it seems|it appears|in my opinion|sort of|kind of|probably|possibly|perhaps|maybe/i,
    apply: (t) =>
      t.replace(
  ...
  },
  {
    ...
    minIntensity: "lite",
    keyword:
      /furthermore|moreover|additionally|in addition|however|nevertheless|nonetheless|therefore|consequently|as a result/i,
    apply: (t) =>
      t
  ...
  // ── ultra: aggressive abbreviations ──
  {
    ...
    apply: (t) => t.replace(CAVEMAN_ABBREV_RE, (m) => CAVEMAN_ABBREV[m.toLowerCase()] ?? m),
  },
];
