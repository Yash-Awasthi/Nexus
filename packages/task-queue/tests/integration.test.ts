...
import {
  WikiUpdatePipeline,
  WikiStore as UpdaterWikiStore,
} from "@nexus/wiki-updater";
...
interface WikiUpdatePayload { documentId: string; content: string; source: string; }
interface SearchIndexPayload { articleId: string; title: string; content: string; tags: string[]; }
...
describe("wiki-updater → task-queue pipeline", () => {
  ...
  beforeEach(() => {
    ...
    taskQueue.task<WikiUpdatePayload>("wiki:update", async (task) => {
      const result = await pipeline.run({
        document: { id: task.payload.documentId, content: task.payload.content, source: task.payload.source },
      });
  ...
  it("enqueues and processes 3 wiki update tasks", async () => {
    taskQueue.enqueue<WikiUpdatePayload>("wiki:update", { documentId: "d1", content: "TypeScript is a strongly-typed superset of JavaScript.", source: "ts.md" });
    taskQueue.enqueue<WikiUpdatePayload>("wiki:update", { documentId: "d2", content: "Nexus is a multi-agent AI platform built with TypeScript.", source: "nexus.md" });
    taskQueue.enqueue<WikiUpdatePayload>("wiki:update", { documentId: "d3", content: "Vitest is a Vite-native testing framework.", source: "vitest.md" });
  ...
  it("retries a flaky handler before permanently failing", async () => {
    ...
    taskQueue.enqueue<WikiUpdatePayload>("wiki:flaky", { documentId: "flaky", content: "x", source: "x" }, { maxRetries: 3 });
...
describe("wiki → search-orchestrator pipeline", () => {
  ...
  it("tracks page versions with diffs", () => {
    const wikiStore = new WikiStore();
    const page = wikiStore.createPage({ slug: "v-page", title: "V Page", content: "v1 content.", createdBy: "yash", tags: [] });
    expect(page.version).toBe(1);

    wikiStore.acl.grant(page.id, "editor", "editor", "yash");
    const updated = wikiStore.updatePage(page.id, { content: "v2 content — expanded.", editedBy: "editor", summary: "Expanded" });
  ...
  it("notifies subscribers on page update", () => {
    const wikiStore = new WikiStore();
    const page = wikiStore.createPage({ slug: "notif-page", title: "Notif", content: "Initial.", createdBy: "alice", tags: [] });
  ...
  it("processes 3 docs: wiki:update → search:index → full-text hits", async () => {
    ...
    queue.task<WikiUpdatePayload>("wiki:update", async (task) => {
      ...
      if (result.articleId) {
        ...
        if (article) queue.enqueue<SearchIndexPayload>("search:index", { articleId: result.articleId, title: article.title, content: article.content, tags: article.tags });
      }
      return result;
    });
    ...
    queue.task<SearchIndexPayload>("search:index", async (task) => {
      try {
        wikiStore.createPage({ slug: `art-${task.payload.articleId}`, title: task.payload.title, content: task.payload.content, createdBy: "bot", tags: task.payload.tags });
      } catch { /* slug collision */ }
      indexed.push({ id: task.payload.articleId, content: task.payload.content, source: "mock", type: "document", score: 0.95, timestamp: new Date().toISOString() });
      return { ok: true };
    });

    const DOCS = [
      { id: "d1", content: "Vitest provides a Vite-native test runner with HMR support.", source: "vitest" },
      { id: "d2", content: "The Nexus knowledge graph uses sha256 deterministic node IDs.", source: "kg" },
      { id: "d3", content: "Redis Streams provide at-most-once delivery semantics.", source: "redis" },
    ];
    for (const d of DOCS) queue.enqueue<WikiUpdatePayload>("wiki:update", { documentId: d.id, content: d.content, source: d.source });
...
describe("search-orchestrator strategy chain", () => {
  it("falls back to next strategy when first is empty", async () => {
    ...
    const fallback = new MockSearchStrategy("mock", {
      results: [{ id: "r1", content: "fallback result", source: "mock" as const, type: "document" as const, score: 0.8, timestamp: new Date().toISOString() }],
    });
  ...
  it("builds timeline segments from multi-day results", async () => {
    ...
    const strategy = new MockSearchStrategy("mock", {
      results: [
        { id: "r1", content: "today", source: "mock" as const, type: "document" as const, score: 0.9, timestamp: now.toISOString() },
        { id: "r2", content: "yesterday", source: "mock" as const, type: "document" as const, score: 0.7, timestamp: yesterday.toISOString() },
      ],
    });
    const orchestrator = new SearchOrchestrator({ chain: new StrategyChain({ strategies: [strategy] }), timelineBuilder: new TimelineBuilder() });
  ...
  it("filters results by minScore", async () => {
    const strategy = new MockSearchStrategy("mock", {
      results: [
        { id: "hi", content: "high", source: "mock" as const, type: "document" as const, score: 0.9, timestamp: new Date().toISOString() },
        { id: "lo", content: "low",  source: "mock" as const, type: "document" as const, score: 0.2, timestamp: new Date().toISOString() },
      ],
    });
    const resp = await new SearchOrchestrator({ chain: new StrategyChain({ strategies: [strategy] }) })
...
describe("obs-providers in-memory end-to-end", () => {
  it("generates observations and handles provider fallback", async () => {
    ...
    registry.register(new MockObservationProvider("primary", "mock-v1", { observation: "User prefers brief answers." }));
  ...
  it("falls back to secondary provider on primary error", async () => {
    ...
    registry.register(new MockObservationProvider("fallback", "v2", { observation: "Fallback obs." }));
...
describe("knowledge-graph in-memory end-to-end", () => {
  it("builds graph, traverses edges, and merges duplicate nodes", async () => {
    const { InMemoryKGStore, KnowledgeGraph, makeNodeId, makeEdgeId } = await import("@nexus/knowledge-graph");
    ...
    await store.upsertNode({ id: aliceId, name: "Alice",    type: "PERSON", confidence: 1.0, properties: {}, sources: ["test"], createdAt: now, updatedAt: now });
    await store.upsertNode({ id: bobId,   name: "Bob",      type: "PERSON", confidence: 1.0, properties: {}, sources: ["test"], createdAt: now, updatedAt: now });
    await store.upsertNode({ id: acmeId,  name: "Acme Corp",type: "ORG",    confidence: 1.0, properties: {}, sources: ["test"], createdAt: now, updatedAt: now });
    ...
    await store.upsertEdge({ id: e1, subjectId: aliceId, predicate: "works_at", objectId: acmeId, confidence: 0.95, sources: ["test"], createdAt: now, updatedAt: now });
    await store.upsertEdge({ id: e2, subjectId: bobId,   predicate: "works_at", objectId: acmeId, confidence: 0.9,  sources: ["test"], createdAt: now, updatedAt: now });
    await store.upsertEdge({ id: e3, subjectId: aliceId, predicate: "mentors",  objectId: bobId,  confidence: 0.8,  sources: ["test"], createdAt: now, updatedAt: now });
    ...
    await store.upsertEdge({ id: e1, subjectId: aliceId, predicate: "works_at", objectId: acmeId, confidence: 0.99, sources: ["enriched"], createdAt: now, updatedAt: now });
