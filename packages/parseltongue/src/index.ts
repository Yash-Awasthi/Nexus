...
export const DEFAULT_TRIGGERS: readonly string[] = [
  "hack", "exploit", "malware", "virus", "ransomware", "phishing", "bypass", "crack",
  "ddos", "sql injection", "xss", "zero day", "rootkit", "keylogger", "botnet",
  "weapon", "bomb", "explosive", "illegal", "piracy", "counterfeit", "fraud",
  "jailbreak", "uncensored", "unrestricted", "ignore instructions", "ignore previous",
  "dan", "do anything now",
];
...
const LEETSPEAK_MAP: Record<string, string> = {
  a: "4", e: "3", i: "1", o: "0", s: "5", t: "7", l: "|", g: "9",
};

const PHONETIC_MAP: Record<string, string> = {
  a: "ay", e: "ee", i: "eye", o: "oh", u: "you",
  c: "see", k: "kay", s: "ess", x: "ex",
};
...
function applyTechnique(word: string, technique: Technique, intensity: Intensity): string {
  ...
  switch (technique) {
    ...
    case "unicode":
      return chars.map((c, i) =>
        i / chars.length < ratio && UNICODE_MAP[c] ? UNICODE_MAP[c]! : c
      ).join("");
    case "zwj":
      return chars.map((c, i) =>
        i > 0 && i / chars.length < ratio ? "\u200d" + c : c
      ).join("");
    case "mixedcase":
      return chars.map((c, i) =>
        i / chars.length < ratio
...
export function getTechniqueDescription(technique: string): string {
  const descriptions: Record<string, string> = {
    ...
    unicode: "Replaces Latin characters with Cyrillic/Greek homoglyphs identical to the eye but distinct in codepoint.",
...
export function applyParseltongue(text: string, config: ParseltongueConfig): ParseltongueResult {
  if (!config.enabled) {
    return { originalText: text, transformedText: text, triggersFound: [], techniqueUsed: config.technique, transformations: [] };
  }
