...
/**
 * GhostStack HTTP server — lightweight in-process HTTP gateway used by
...
class GhostStackHttpServer implements GhostStackServer {
  private server: http.Server | null = null;
  private _running = false;
  private readonly startedAt = Date.now();
  ctx?: GhostStackServer["ctx"];
...
/**
 * Create a GhostStack HTTP server instance.
 *
 * @param _repoRoot Repo root (reserved for future config loading).
 * @param port      Port to listen on (default: 3000).
 */
export function createGhostStackServer(_repoRoot: string, port = 3000): Promise<GhostStackServer> {
  return Promise.resolve(new GhostStackHttpServer(port));
}
