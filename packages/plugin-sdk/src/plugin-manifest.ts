...
export function validatePluginManifest(input: unknown): PluginManifest {
  ...
  if (!Array.isArray(caps)) {
    ...
    caps.forEach((c, i) => {
      ...
      else if (!KNOWN_CAPABILITY_SET.has(c)) issues.push(`capabilities[${i}]: unknown capability "${c}"`);
    });
  ...
  if (m["config"] !== undefined) {
    if (!Array.isArray(m["config"])) {
      ...
      (m["config"] as unknown[]).forEach((f, i) => {
        ...
        if (!isNonEmptyString(field["key"])) issues.push(`config[${i}].key: required non-empty string`);
      });
    }
  }
