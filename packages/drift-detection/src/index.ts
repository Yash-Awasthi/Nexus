...
export class DriftDetector {
  ...
  checkDrift(modelId: string): DriftReport {
    ...
    const baseline = results.slice(-this.config.baselineWindowSize - this.config.testWindowSize, -this.config.testWindowSize);
    ...
    for (const metric of [...metrics, ...Array.from(allMetricKeys)]) {
      const baselineValues = baseline.map((r) => this.extractMetric(r, metric)).filter((v) => v !== null) as number[];
      const testValues = test.map((r) => this.extractMetric(r, metric)).filter((v) => v !== null) as number[];
      ...
      const percentageChange = baselineMean !== 0 ? (Math.abs(change) / Math.abs(baselineMean)) * 100 : 0;
      ...
      if (isStatisticallySignificant || exceedsAbsolute || exceedsPercentage) {
        const severity = isStatisticallySignificant && exceedsAbsolute ? 'critical'
          : isStatisticallySignificant || exceedsPercentage ? 'warning'
          : 'info';

        alerts.push({
          metric,
          type: isStatisticallySignificant ? 'statistical' : exceedsAbsolute ? 'absolute' : 'percentage',
          baselineMean,
          testMean,
          change,
          pValue,
   ...
   */
  summary(modelId: string): { totalResults: number; latestTimestamp: number; avgLatency: number; avgErrorRate: number } {
    const results = this.results.get(modelId) ?? [];
    if (results.length === 0) return { totalResults: 0, latestTimestamp: 0, avgLatency: 0, avgErrorRate: 0 };
  ...
  private extractMetric(result: EvalResult, metric: string): number | null {
    switch (metric) {
      case 'latencyMs': return result.latencyMs;
      case 'tokenCount': return result.tokenCount;
      case 'errorRate': return result.errorRate;
      default: return result.scores[metric] ?? null;
    }
  }
}
