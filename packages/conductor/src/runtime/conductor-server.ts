...
import { registerConductorMcpBridge } from "../orchestration/conductor-mcp-bridge";
...
export async function createConductorServer(repoRoot: string): Promise<ConductorServer> {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      ctx.logger.error("Conductor HTTP handler error", err);
...
export async function startHttpServer(): Promise<http.Server> {
  ...
  const { loadConductorConfig } = await import("./conductor-config");
