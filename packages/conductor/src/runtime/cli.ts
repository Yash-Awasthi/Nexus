...
import { loadConductorConfig } from "./conductor-config";
...
import { createConductorServer } from "./conductor-server";
...
Commands:
  ...
  version              Print Conductor version and runtime info
  help                 Show this help
...
async function cmdInit(): Promise<void> {
  console.log("\n=================== Conductor Developer Init Wizard ===================");
  ...
  // Create conductor.config.json if not exists
...
async function cmdDiagnose(): Promise<void> {
  const config = loadConductorConfig(repoRoot);
  console.log("=== Conductor diagnose ===\n");
...
async function cmdStart(federation: boolean): Promise<void> {
  ...
  console.log(`[Conductor] API http://127.0.0.1:${gs.port}`);
...
async function cmdRestart(): Promise<void> {
  console.log("[gs] Restarting Conductor federation...");
  await cmdStop();
