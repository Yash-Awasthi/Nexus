...
export async function bootstrap() {
  ...
  console.log(
    "  \\_____|_| |_|\\___/|___/\\__|_____/ \\__\\__,_|\\___|_|\\_\\      \\/     |_|  |_|",
  );
  console.log("                                                                               ");
  ...
  console.log(
    `[BOOT] Loaded ${ctx.registry.listWorkflows().length} workflow definition(s) from specs/`,
  );

  const activeServices = await startRuntime(ctx);
  console.log(
    `[BOOT] Active orchestration services loaded successfully: ${activeServices.join(", ")}`,
  );
  ...
  const safeWorkflow = browserTemplate.createWorkflow({
    id: "showcase-safe-research",
    limitBytes: 5000,
  });
  ctx.registry.registerWorkflow(safeWorkflow);

  console.log("[SHOWCASE] Executing SAFE Workflow...");
  const safeExecResult = await ctx.workflowEngine.executeWorkflow(
    "showcase-safe-research",
    "exec-safe-demo",
  );
  console.log(
    `[SHOWCASE] SAFE Workflow finished with status: \x1b[32m${safeExecResult.status}\x1b[0m`,
  );
  ...
  const illegalExecResult = await ctx.workflowEngine.executeWorkflow(
    "showcase-illegal-research",
    "exec-illegal-demo",
  );
  console.log(
  ...
  const approvalWorkflow = browserTemplate.createWorkflow({
    id: "showcase-approval-research",
    limitBytes: 25000,
  });
  ctx.registry.registerWorkflow(approvalWorkflow);

  console.log("[SHOWCASE] Executing SECURE Workflow...");
  const approvalExecResult = await ctx.workflowEngine.executeWorkflow(
    "showcase-approval-research",
    "exec-approval-demo",
  );
  console.log(
  ...
  const approvedResult = await ctx.workflowEngine.approveAndTriggerWorkflow(
