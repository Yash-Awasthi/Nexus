...
import type { ConductorRuntimeContext } from "../runtime/runtime-context";
...
/**
 * In-process MCP transport exposing Conductor orchestrator capabilities.
 */
export class ConductorMcpBridge implements IMCPTransport {
  private connected = false;

  constructor(private readonly ctx: ConductorRuntimeContext) {}
  ...
  async send(message: { method: string; params?: Record<string, unknown> }): Promise<unknown> {
    if (!this.connected) {
      throw new Error("Conductor MCP bridge not connected");
    }
  ...
  private async dispatchTool(name: string, args: Record<string, unknown>): Promise<string> {
    switch (name) {
      ...
      case "conductor_runtime_snapshot": {
        const snap = await this.ctx.inspector.getSnapshots();
        return JSON.stringify(snap, null, 2);
      }
      case "conductor_list_workflows": {
      ...
      case "conductor_execute_workflow": {
      ...
      case "conductor_workflow_cancel": {
      ...
      case "conductor_workflow_resume": {
        const executionId = args.executionId as string;
        if (!executionId) throw new Error("executionId is required");
      ...
      case "conductor_workflow_checkpoints": {
      ...
      case "conductor_workflow_replay": {
        const executionId = args.executionId as string;
        if (!executionId) throw new Error("executionId is required");
      ...
      case "conductor_memory_stats": {
        const stats = await this.ctx.memoryStore.getStats();
        return JSON.stringify(stats, null, 2);
      }
      case "conductor_memory_store": {
      ...
      default:
        throw new Error(`Unknown Conductor MCP tool: ${name}`);
    }
  }
}

export async function registerConductorMcpBridge(ctx: ConductorRuntimeContext): Promise<{
  registry: MCPServerRegistry;
  runtime: MCPRuntime;
}> {
  const registry = new MCPServerRegistry();
  const transport = new ConductorMcpBridge(ctx);
...
export const GHOSTSTACK_MCP_TOOLS = [
  "conductor_health",
  "conductor_runtime_snapshot",
  "conductor_list_workflows",
  "conductor_load_spec",
  "conductor_execute_workflow",
  "conductor_workflow_cancel",
  "conductor_workflow_resume",
  "conductor_workflow_checkpoints",
  "conductor_workflow_replay",
  "conductor_run_e2e",
  "conductor_floci_execute",
  "conductor_sandbox_write",
  "conductor_sandbox_read",
  "conductor_sandbox_list",
  // Memory & Knowledge Layer
  "conductor_memory_stats",
  "conductor_memory_store",
  "conductor_memory_query",
  // Agent Bus
  "conductor_agent_capabilities",
  "conductor_agent_send",
  "conductor_agent_find",
  // Circuit Breaker
  "conductor_circuit_state",
  "conductor_circuit_reset",
  // Extended Floci Operations
  "conductor_floci_extended",
  // Diagnostics
  "conductor_diagnostics",
  "conductor_health_history",
  // Runtime Graph
  "conductor_runtime_graph",
];
