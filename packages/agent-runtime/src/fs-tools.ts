...
function asPosInt(v: unknown, fallback: number): number {
  ...
  if (!Number.isFinite(n) || n < 0)
    throw new Error(`expected a non-negative number, got ${String(v)}`);
  return Math.floor(n);
}
  ...
  return {
    ...
    async handler(args, ctx) {
      ...
      if (stat.size > MAX_READ_BYTES) {
        throw new Error(
          `file too large (${stat.size} bytes > ${MAX_READ_BYTES} cap): ${String(args.path)}`,
        );
      }
  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        glob: {
          type: "string",
          description: "Only scan files whose relative path matches this glob.",
        },
...
export function createRunCommandTool(exec: CommandExecutor): RuntimeTool {
  return {
    ...
    async handler(args, ctx) {
      ...
      const timeoutMs =
        args.timeout_ms === undefined ? undefined : asPosInt(args.timeout_ms, 0) || undefined;
