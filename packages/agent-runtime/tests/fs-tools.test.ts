...
describe("read_file", () => {
  ...
  it("honours offset/limit line slicing", async () => {
    const out = await tools().read_file.handler(
  ...
  it("rejects a path escaping the workspace", async () => {
    await expect(
      tools().read_file.handler({ path: "../outside" }, { workingDir: root }),
    ).rejects.toThrow(/escapes workspace/);
  });

  it("throws when no workspace root is set", async () => {
    await expect(tools().read_file.handler({ path: "readme.md" }, {})).rejects.toThrow(
      /workspace root/,
    );
  });
});
...
describe("list_files", () => {
  ...
  it("recurses and skips ignored dirs", async () => {
    const out = (await tools().list_files.handler(
      { recursive: true },
      { workingDir: root },
    )) as string[];
...
describe("glob", () => {
  it("finds files matching a `**` pattern, sorted, skipping node_modules", async () => {
    const out = (await tools().glob.handler(
      { pattern: "**/*.ts" },
      { workingDir: root },
    )) as string[];
...
describe("grep", () => {
  ...
  it("filters scanned files by glob", async () => {
    const out = (await tools().grep.handler(
      { pattern: "k", glob: "**/*.json" },
      { workingDir: root },
    )) as Array<{
      file: string;
...
describe("edit_file", () => {
  ...
  it("throws when old_string is absent", async () => {
    ...
    await expect(
      createEditFileTool().handler(
        { path: "n.txt", old_string: "zzz", new_string: "q" },
        { workingDir: ws },
      ),
...
describe("run_command", () => {
  it("is gated (requires_permission, not auto-allowed)", () => {
    const t = createRunCommandTool(async () => ({
      stdout: "",
      stderr: "",
      exitCode: 0,
      timedOut: false,
  ...
  it("rejects an already-aborted signal before executing", async () => {
    ...
    ctrl.abort();
    await expect(
      createRunCommandTool(exec).handler(
        { command: "x" },
        { workingDir: "/ws", signal: ctrl.signal },
      ),
    ).rejects.toThrow(/aborted/);
